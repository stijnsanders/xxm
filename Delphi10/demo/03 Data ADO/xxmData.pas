...
type
  ...
  private
    ...
    procedure ReadQueriesSQL(const ProjectFolder: string);
  public
    constructor Create(const ProjectFolder: string);
    destructor Destroy; override;
    function GetSQL(const QueryName: string): string;
  end;
  ...
  public
    constructor Create(const QueryName: string; const Values: array of Variant); overload;
    constructor Create(Recordset: Recordset); overload;
    constructor Create(const SQL: string); overload;
    destructor Destroy; override;

    class function SingleValue(const QueryName: string; const Values: array of Variant): Variant; overload;
    class function SingleValue(const SQL: string): Variant; overload;
  ...
  public
    constructor Create(const TableName, PKName: string; const Id: Variant);
    destructor Destroy; override;

    class function Execute(const QueryName: string; const Values: array of Variant): integer; overload;
...
uses Variants, IOUtils, ComObj, xxmSession;

{ TQueryStore }

constructor TQueryStore.Create(const ProjectFolder: string);
begin
  inherited Create;
  ReadQueriesSQL(ProjectFolder);
end;
...
function TQueryStore.GetSQL(const QueryName: string): string;
var
  i,l:integer;
begin
...
procedure TQueryStore.ReadQueriesSQL(const ProjectFolder: string);
var
  s:string;
  i,l,q,ql,r1,r2,s1,s2,n1,n2:integer;
begin
  s:=TFile.ReadAllText(ProjectFolder+'queries.sql');
  l:=Length(s);
  i:=1;
  q:=0;
  ql:=0;
...
function ErrInfo(const QueryName: string; const Values: array of Variant): string;
var
  i,l:integer;
begin
  l:=Length(Values);
...
constructor TQueryResult.Create(const QueryName: string; const Values: array of Variant);
var
  cmd:Command;
begin
  inherited Create;
...
constructor TQueryResult.Create(const SQL: string);
begin
  inherited Create;
  FFirstRead:=true;
  //FRecordSet:=Session.DbCon.Execute(,v,adCmdText);
...
class function TQueryResult.SingleValue(const QueryName: string; const Values: array of Variant): Variant;
var
  cmd:Command;
  rs:Recordset;
  v:OleVariant;
...
class function TQueryResult.SingleValue(const SQL: string): Variant;
var
  rs:Recordset;
begin
...
constructor TDataChanger.Create(const TableName, PKName: string; const Id: Variant);
var
  sql:string;
  idX:int64;
begin
  ...
  FRecordSet.Open(sql,
    ...
    adCmdText);
  if VarIsNull(Id) or (Id=Variant(0)) then FRecordSet.AddNew(EmptyParam,EmptyParam);
  //else editmode?
end;
...
class function TDataChanger.Execute(const QueryName: string; const Values: array of Variant): integer;
var
  cmd:Command;
  v:OleVariant;
begin
